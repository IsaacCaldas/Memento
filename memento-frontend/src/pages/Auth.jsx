...
export default function Auth() {
  ...
  async function handleSign() {
    ...
    let sign_route = new_user ? 'signup' : 'signin'

    if (new_user) {
      await axios.post(`http://localhost:3000/${sign_route}`, { email, password, name }).then(res => {
  ...
  return (
    <ImageBackground source={bg_image} style={styles.background}>
      <Container filter="blur(2px)" bg_color='#00000055' width='100%'>
        ...
        <Form height={600} padding={15} bg_color='#000000aa'>
          <Label color="#efefef" size={35} weight='bold' bottom={20}>
            {new_user ? 'Cadastrar' : 'Entrar'}
          </Label>
          {new_user && <Input
          ...
          style={{marginVertical: 10, width: '100%'}}>
            <ButtonLabel bold>{new_user ? 'CADASTRAR' : 'ENTRAR'}</ButtonLabel>
          </Button>
          { new_user ? 
